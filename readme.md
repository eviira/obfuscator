...
- the next payload key is gotten by just generating another random value after encrypting the payload, this has 2 main implcations:
  - if the program hits a known hash, you can also predict what the next hash is going to be
  - the total amount of hashes the program can have is RAND_MAX
